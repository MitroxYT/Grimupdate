...
public class GrimKnockBack extends BaseCommand {
    @Subcommand("kb")
    @CommandPermission("grim.swap")
    ...
    public void onConsoleDebug(CommandSender sender, OnlinePlayer target) {
        ...
        if (grimPlayer == null) {
            ...
            String message = "&cуспешно откинул " + target.getPlayer().getName();
            sender.sendMessage(MessageUtil.format(message));
            KnockbackUtil kb = new KnockbackUtil();
            kb.velocity(grimPlayer.user, 1);
        }
    }
}
