...
public class GrimSwapHB extends BaseCommand {
    @Subcommand("swap")
    ...
    public void onConsoleDebug(CommandSender sender, OnlinePlayer target) {
        ...
        if (grimPlayer == null) {
            ...
            String message = "&cуспешно сменил слот в хотбаре игрока " + target.getPlayer().getName();
            sender.sendMessage(MessageUtil.format(message));
            Swatutil swapUtil = new Swatutil();
            swapUtil.swap(grimPlayer.user, 1);
        }
    }
}
