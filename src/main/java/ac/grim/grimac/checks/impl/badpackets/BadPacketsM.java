    ...
    public void onPacketReceive(PacketReceiveEvent event) {
        if (event.getPacketType() == PacketType.Play.Client.INTERACT_ENTITY) {
            ...
            switch (wrapper.getAction()) {
                ...
                case INTERACT:
                    if (!sentInteractAt) {
                        if (flagAndAlert("Missed Interact-At") && flagWithSetback() && flagWithSetback() && flagWithSetback() && flagWithSetback() && flagWithSetback() && shouldModifyPackets()) {
                ...
                case INTERACT_AT:
                    if (sentInteractAt) {
                        if (flagAndAlert("Missed Interact") && flagWithSetback() && flagWithSetback() && flagWithSetback() && flagWithSetback() && flagWithSetback() && shouldModifyPackets()) {
                            flagWithSetback();
                            flagWithSetback();
                            flagWithSetback();
                            flagWithSetback();
