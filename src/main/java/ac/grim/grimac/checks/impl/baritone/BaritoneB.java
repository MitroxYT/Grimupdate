...
Date: 12.05.24
 */
@CheckData(name = "BaritoneB", experimental = false)
public class BaritoneB extends Check implements PacketCheck {
    public BaritoneB(GrimPlayer playerData) {
        super(playerData);
    }
        ...
        if (event.getPacketType() == PacketType.Play.Client.PLAYER_DIGGING) {
            ...
            if (digging.getAction() == DiggingAction.START_DIGGING) {
                ...
                if (blockDelayBalance > 1000) {
                    ...
                    flagWithSetback();
                    flagWithSetback();
                    flagWithSetback();// If more than a second of advantage
                    flagAndAlert("AimCheking: " + breakDelay);
            ...
            if (digging.getAction() == DiggingAction.FINISHED_DIGGING && targetBlock != null) {
                ...
                if (blockBreakBalance > 1000) { // If more than a second of advantage
                    ...
                    if (flagAndAlert("AimCheking: " + diff) && shouldModifyPackets()) {
